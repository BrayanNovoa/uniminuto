...
CREATE TABLE tb_estudiantes(estudiante_id INT(11) NOT NULL AUTO_INCREMENT, programa_id INT(3) NOT NULL, INDEX programa_ind(programa_id), FOREIGN KEY(programa_id) REFERENCES tb_programas(programa_id) ON DELETE CASCADE, periodo_id INT(3) NOT NULL, INDEX periodo_ind(periodo_id), FOREIGN KEY(periodo_id) REFERENCES tb_periodos(periodo_id) ON DELETE CASCADE, estudiante_cod VARCHAR(12) NOT NULL, INDEX estudiante_ind1 (estudiante_cod), PRIMARY KEY(estudiante_id));
