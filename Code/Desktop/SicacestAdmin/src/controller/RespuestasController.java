...
public class RespuestasController implements interfaces.IRespuestas{
    ...
    public void obtenerRespuestas(String param){
        sql="SELECT R.respuesta_id AS CODIGO_RESP, R.estudiante_cod, E.encuesta, P.pregunta, R.respuesta FROM tb_encuestas AS E, tb_preguntas AS P, tb_respuestas AS R WHERE E.encuesta_id=P.encuesta_id AND P.pregunta_id=R.pregunta_id ORDER BY respuesta_id, R.estudiante_cod;";
    ...
    public void buscarPosiblesRespuestas(String pregunta){
        ...
        registro = new String[campos.length];
        sql = "SELECT p.pregunta, r.posible_respuesta FROM tb_preguntas AS p, tb_posibles_respuestas AS r WHERE p.pregunta_id=r.pregunta_id AND p.pregunta='"+pregunta+"';";
        model = new DefaultTableModel(null,campos);
        try{
